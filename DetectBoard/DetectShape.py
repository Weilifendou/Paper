...
def CutBoard(image):
    ...
    # cv2.imshow('sat', sat)
    # cv2.imshow('val', val)
    _, binary = cv2.threshold(val, 80, 255, cv2.THRESH_BINARY)  # 根据实际情况调整
    kernel = np.ones((5, 5), np.uint8)
    closed = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)
...
def OpenCamera():
    global path
    path = os.path.join(path, 'Pictures', '3.jpg')
    print(path)  # 获得当前工作目录
    raw = cv2.imread(path)
    raw = cv2.resize(raw, (1200, 800))
    board = CutBoard(raw)
    # 将OpenCV图像转换为Qt图像格式
    height, width, channel = board.shape
    bytes_per_line = 3 * width
    q_image = QtGui.QImage(board.data, width, height, bytes_per_line, QtGui.QImage.Format_RGB888).rgbSwapped()
    # 将Qt图像设置到QLabel中
